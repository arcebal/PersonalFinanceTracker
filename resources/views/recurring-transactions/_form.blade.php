...
<form method="POST" action="{{ $action }}" class="auth-form">
    ...
    <div class="panel-heading mb-6">
        <div class="panel-title-block">
            ...
            <p class="panel-subtitle">Set a repeating income or expense, choose when it becomes due, and decide how early
                the reminder should appear.</p>
        </div>
    </div>
    ...
    <div class="form-grid">
        <div class="form-field md:col-span-6">
            ...
            <input type="text" name="description" value="{{ old('description', $item?->description) }}"
                placeholder="Rent, Salary, Netflix, Utilities" required>
            @error('description')
            ...
            <select name="account_id">
                @foreach ($accounts as $account)
                    <option value="{{ $account->id }}"
                        {{ (string) old('account_id', $item?->account_id) === (string) $account->id ? 'selected' : '' }}>
        ...
        <div class="form-field md:col-span-4">
            ...
            <input type="number" name="interval" min="1" max="12"
                value="{{ old('interval', $item?->interval ?? 1) }}" required>
            @error('interval')
                <p class="text-sm text-expense">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="form-field md:col-span-4">
            ...
            <input type="date" name="start_date"
                value="{{ old('start_date', optional($item?->start_date)->format('Y-m-d') ?? now()->format('Y-m-d')) }}"
                required>
            @error('start_date')
                <p class="text-sm text-expense">{{ $message }}</p>
            @enderror
        ...
        <div class="form-field md:col-span-4">
            ...
            <input type="date" name="next_due_date"
                value="{{ old('next_due_date', optional($item?->next_due_date)->format('Y-m-d') ?? now()->format('Y-m-d')) }}"
                required>
            @error('next_due_date')
                <p class="text-sm text-expense">{{ $message }}</p>
            @enderror
        ...
        <div class="form-field md:col-span-4">
            ...
            <input type="date" name="ends_on"
                value="{{ old('ends_on', optional($item?->ends_on)->format('Y-m-d')) }}">
            @error('ends_on')
                <p class="text-sm text-expense">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="form-field md:col-span-6">
            ...
            <input type="number" name="reminder_days_before" min="0" max="14"
                value="{{ old('reminder_days_before', $item?->reminder_days_before ?? 3) }}" required>
        ...
        <div class="form-field md:col-span-6">
            ...
            <label class="settings-choice-card">
                <input type="checkbox" name="is_active" value="1" class="settings-choice-input"
                    {{ old('is_active', $item?->is_active ?? true) ? 'checked' : '' }}>
                <span class="settings-choice-copy">
                    <span class="settings-choice-title">Active schedule</span>
                    <span class="settings-choice-note">Paused items stay saved but do not show reminders or due
                        actions.</span>
                </span>
            </label>
        </div>
