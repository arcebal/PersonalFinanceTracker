    ...
    <div class="page-shell">
        <section class="page-header">
            <div class="page-title-block">
                ...
                <p class="page-subtitle">Review income and expense activity, then narrow the table with account, category,
                    and date filters.</p>
            </div>
            <div class="page-actions">
                <a href="{{ route('transactions.trashed') }}" class="btn-secondary" title="View trash">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-field md:col-span-3">
                        ...
                        <select name="category_id">
                            ...
                            @foreach ($categories as $c)
                                <option value="{{ $c->id }}"
                                    {{ isset($filters['category_id']) && $filters['category_id'] == $c->id ? 'selected' : '' }}>
                                    {{ $c->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-field md:col-span-3">
                        ...
                        <select name="account_id">
                            ...
                            @foreach ($accounts as $a)
                                <option value="{{ $a->id }}"
                                    {{ isset($filters['account_id']) && $filters['account_id'] == $a->id ? 'selected' : '' }}>
                                    {{ $a->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-field md:col-span-3">
                        ...
                        <select name="type">
                            <option value="">All</option>
                            <option value="income"
                                {{ isset($filters['type']) && $filters['type'] == 'income' ? 'selected' : '' }}>Income
                            </option>
                            <option value="expense"
                                {{ isset($filters['type']) && $filters['type'] == 'expense' ? 'selected' : '' }}>Expense
                            </option>
                        </select>
                    </div>
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            @foreach ($transactions as $t)
                                <tr>
                                    <td>{{ \Illuminate\Support\Carbon::parse($t->transaction_date)->format('M d, Y') }}
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <span class="table-badge"
                                            style="border-color: {{ $t->category->color }}33; background-color: {{ $t->category->color }}1f; color: {{ $t->category->color }};">
                                            {{ $t->category->name }}
                                        </span>
                                    </td>
