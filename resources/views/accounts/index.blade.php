    ...
    <div class="page-shell">
        <section class="page-header">
            <div class="page-title-block">
                ...
                <p class="page-subtitle">Keep cash, wallet, and e-wallet balances organized so transactions can be assigned
                    cleanly.</p>
            </div>
            <div class="page-actions">
                <a href="{{ route('accounts.trashed') }}" class="btn-secondary" title="View trash">
        ...
        <section class="section-card">
            <div class="grid grid-cols-2 divide-x divide-[var(--border-subtle)]">
                ...
                <div class="pl-6">
                    ...
                    <div class="metric-note">Total available money across all accounts.</div>
                </div>
            </div>
        </section>
        ...
        @if ($accounts->count())
            <section class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
                @foreach ($accounts as $acc)
                    <article class="section-card account-card cursor-pointer" data-account-id="{{ $acc->id }}"
                        data-account-name="{{ $acc->name }}" data-account-balance="{{ $acc->balance }}">
                        ...
                        <div class="page-actions mt-6">
                            <a href="{{ route('accounts.edit', $acc) }}" onclick="event.stopPropagation()"
                                class="btn-secondary flex-1">Edit</a>
                            <form action="{{ route('accounts.destroy', $acc) }}" method="POST"
                                class="swal-delete-form flex-1">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="delete-btn btn-danger w-full"
